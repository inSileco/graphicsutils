...
#'
#' @param side a numerical or character vector or a character string
#' specifying which side(s) of the plot the box is to be drawn (see details).
...
#' color, `col` and for \code{type = 'b'}, `pch`. Also the line characteristics
...
#' @export
#'
#' @details This function intends to give more flexibility to the
#' [graphics::box()] function. As `which` parameter, the user provides an
#' object first coerced by `as.character` to a character string that is
#' secondly split into single characters. For all of these characters, matches
#' are sought with all elements of `1, 2, 3, 4, b, l, t, r` where `1=below,
#' 2=left, 3=above, 4=right, b=below, l=left, t=above and r=right`.
#'
#'
...
box2 <- function(side, which = c("plot", "figure", "outer", "inner"),
  ...
  if (length(ax)) {
    ...
    warning("'side' does not match ant of the available values.")
  }
