...
pickcolors <- function(colors=6, ramp=rainbow(1024)){
    ...
    image(matrix(1:length(ramp)), col=ramp, axes=FALSE, ann=FALSE)
    box(lwd=3, col="white")
    ##
    col_ini <- 3
    shades <- colorRampPalette(c("white",col_ini,"black"))(1024)
    par(fig=c(0,1,0.6,0.8), new=TRUE)
    image(matrix(1:length(shades)), col=shades, axes=FALSE, ann=FALSE)
    box(lwd=3, col="white")
    ##
