...
ganttChart <- function(df, type = "all", col = gpuPalette("insileco")[-1], perc_txt = 20,
    order = T) {
    ...
    text(x = prt, y = rep_len(.9, length(prt)), labels = prt, pos = 2, srt = 90)
    par(lend = 1)
...
# data(dfGantt)
# par(lend = 1) plotMilestone(df, perc_txt = 40)
ganttChart(dfGantt)
