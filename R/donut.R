...
donut <- function(vec, eaten = 0, labels = NULL, rot = 0, cex = 0.8, tck = 0.05, 
    ...
    ## labels
    if (is.null(labels)) 
        labels <- as.character(1:nb) else labels <- as.graphicsAnnot(labels)
    ## Dimension
    ...
    for (i in 1L:nb) {
        ...
        vy <- c(sin(xb[sx]), width * sin(xb[sx]), width * sin(xb[sx:(sx + vecb[i])]), 
            sin(xb[(sx + vecb[i]):sx]))
        if (!is.null(density)) 
            polygon(cx + cex.x * vx, cy + cex.y * vy, col = mycol[i], border = border, 
                density = density[i], angle = angle[i]) else polygon(cx + cex.x * vx, cy + cex.y * vy, col = mycol[i], border = border)
        ...
        if (!is.na(lab) && nzchar(lab)) {
            ...
            lines(cx + cex.x * c(1, 1 + tck) * cos(xb[mid]), cy + cex.y * c(1, 1 + 
                tck) * sin(xb[mid]), ...)
            text(cx + (1 + tck) * cex.x * cos(xb[mid]), cy + cex.y * (1 + tck) * 
    ...
    if (eaten) {
        lines(cx + cex.x * c(cos(xb[1]), width * cos(xb[1])), cy + cex.y * c(sin(xb[1]), 
            width * sin(xb[1])), ...)
        lines(cx + cex.x * c(cos(xb[szb]), width * cos(xb[szb])), cy + cex.y * c(sin(xb[szb]), 
            width * sin(xb[szb])), ...)
    }
}
