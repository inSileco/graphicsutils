...
donut <- function(vec, eaten = 0, labels = NULL, rot = 0, cex = 0.8,
    ...
    for (i in seq_len(nb)) {
        ...
        vy <- c(sin(xb[sx]), width * sin(xb[sx]), width * sin(xb[sx:(sx + vecb[i])]),
            sin(xb[(sx + vecb[i]):sx]))
        if (!is.null(density)) {
          polygon(cx + cex.x * vx, cy + cex.y * vy, col = mycol[i],
            border = border, density = density[i], angle = angle[i])
        } else polygon(cx + cex.x * vx, cy + cex.y * vy, col = mycol[i],
            border = border)
    ...
    if (eaten) {
        lines(cx + cex.x * c(cos(xb[1L]), width * cos(xb[1L])),
          cy + cex.y *c(sin(xb[1L]), width * sin(xb[1L])), ...)
        lines(cx + cex.x * c(cos(xb[szb]), width * cos(xb[szb])),
