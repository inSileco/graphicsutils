...
pickcolors2 <- function(colors=6, ramp=rainbow(1024)){
    ...
    while (slc<colors){
        ...
        while (tone==0){
            ...
            xy <- locator(1L)
            ## ---
            if (abs(xy$y)>1 | xy$x>1 | xy$x<0) cat('Your click is outside the red frame! \n')
            else {
                box(lwd=2.8)
                ...
                while (xy0$x>1 | xy0$x<0 | (xy0$y)^2>1) {
                    cat('Your click is outside the red frame! \n')
                    xy0 <- locator(1L)
                }
                if (xy0$y>0) {
                    box(lwd=2.8)
        ...
        while (!slc2) {
            ...
            ## ----
            if (xy2$y> 8.5 | xy2$y< -0.5) cat('Your click is outside the red frame! \n')
            else {
                box(lwd=2.8)
                ##
                diffx <- ((1:16)-xy2$x)^2
