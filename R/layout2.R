...
layout2 <- function(n = 4, grain.x = 20, grain.y = grain.x, getmatrix = TRUE, show = TRUE, 
    ...
    ## --- plot and lines
    par(mfrpw = c(1, 1), mar = rep(0, 4), xaxs = "i", yaxs = "i", ann = T)
    ...
    if ((grain.x%%2) == 0 && (grain.y%%2) == 1) {
        points(x = rep(seqx[1 + grain.x/2], 2), y = seqy[c(1 + grain.y/3, 1 + 2 * 
            grain.y/3)], pch = 43)
    }
    if ((grain.x%%2) == 1 && (grain.y%%2) == 0) {
        points(x = seqx[c(1 + grain.x/3, 1 + 2 * grain.x/3)], y = rep(seqy[1 + grain.y/2], 
            2), pch = 43)
    }
    if ((grain.x%%2) == 1 && (grain.y%%2) == 1) {
        points(x = seqx[c(1 + grain.x/3, 1 + 2 * grain.x/3)], y = seqy[c(1 + grain.y/3, 
            1 + 2 * grain.y/3)], pch = 43)
        points(x = seqx[c(1 + 2 * grain.x/3, 1 + grain.x/3)], y = seqy[c(1 + grain.y/3, 
            1 + 2 * grain.y/3)], pch = 43)
    }
    rx <- max(which((grain.x%%1:5) == 0))
    ry <- max(which((grain.y%%1:5) == 0))
    if (rx > 1) 
        abline(v = seqx[1 + seq(grain.y/rx, (rx - 1) * grain.x/rx, length.out = rx - 
            1)])
    if (ry > 1) 
        abline(h = seqy[1 + seq(grain.y/ry, (ry - 1) * grain.y/ry, length.out = ry - 
            1)])
    ## ---- center sequences
    ...
    for (i in 1:n) {
        ...
        rect(seqx[xlf], seqy[ybt], seqx[xrg + 1], seqy[ytp + 1], col = "#00000088", 
            border = "#00000088")
    }
    dev.off()
    par(old.par)
