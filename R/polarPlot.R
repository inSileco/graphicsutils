...
polarPlot <- function(seqtime, seqval = NULL, rad = 1, from = 0, to = 2 * pi, incr = 0.005,
    ...
    if (!is.na(labelr) && nzchar(labelr)) {
        graphics::text(rep(0, 6), -0.2 * 0:5, grDevices::as.graphicsAnnot(labelr), cex = 1.1,
            pos = 3)
    }
    ...
    if (!is.na(labelc) && nzchar(labelc)) {
        if (mangle[1]%%(2 * pi) == mangle[length(mangle)]%%(2 * pi)) {
            ...
            graphics::text(1.15 * cos(mg1), 1.15 * sin(mg1), labelc[1], pos = 4, cex = 1.15,
                col = tckcol)
            graphics::text(1.15 * cos(mgl), 1.15 * sin(mgl), labelc[length(labelc)], pos = 2,
                cex = 1.15, col = tckcol)
            mangle <- mangle[-c(1L, length(mangle))]
            labelc <- labelc[-c(1L, length(labelc))]
        }
        graphics::text(1.15 * cos(mangle), 1.15 * sin(mangle), labelc, cex = 1.15, col = tckcol)
    }
