...
#' @param seqval sequence of values of interest (radial axis, equivalent to
#' the y axis).
...
#' @param atc the points at which tick-marks are to be drawn. By default (when
#' `NULL`) tickmark locations are computed.
#' @param labelc character or expression vector of labels to be placed at the
#' tickpoints.
#' @param atr the points at which radial-axis marks are to be drawn.
#' @param tckcol color of the tickmarks.
#' @param labelr character or expression vector specifying the _text_ to be
#' placed at the radial-axis marks.
#' @param clockwise logical. If TRUE, the plot must de read clockwise,
#' otherwise, counter-clockwise.
#' @param n_signif number of significant numbers to be displayed (used when
#' labelc is `NULL`).
...
polarPlot <- function(seqtime, seqval = NULL, rad = 1, from = 0, to = 2 * pi,
    ...
    ##
    seqtp <- -0.5 * pi + from + (to - from) * (seqtime - rgtime[1L]) / 
        (rgtime[2L] - rgtime[1L])
    if (clockwise) {
        seqtp <- 2 * from - seqtp
    }
    ...
    # circles
    for (i in rad / 5 * seq_len(5 - 1)) {
        circles(0, 0, rad * i, col = NA, lwd = 0.5 +
            0.5 * i, lty = 2)
    }
    ...
    points(seqvp[1L] * cos(seqtp[1L]), seqvp[1L] * sin(seqtp[1L]),
        pch = 19,
        cex = 1.2
    )
    lv <- length(seqval)
    ...
    graphics::segments(cos(mangle), sin(mangle), 1.05 * cos(mangle),
        1.05 * sin(mangle))
    ...
    if (is.null(labelc)) {
        ...
        labelc <- as.graphicsAnnot(labelc)
    }
    if (!any(is.na(labelc))) {
        if (mangle[1L] %% (2 * pi) == mangle[length(mangle)] %% (2 * pi)) {
            mg1 <- mangle[1L]
            mgl <- mangle[length(mangle)]
            graphics::text(1.15 * cos(mg1), 1.15 * sin(mg1), labelc[1L],
                pos = 4,
                cex = 1.15, col = tckcol
            )
        ...
        text(1.15 * cos(mangle), 1.15 * sin(mangle), labelc,
            cex = 1.15,
            col = tckcol
        )
    }
